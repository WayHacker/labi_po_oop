...
public:
    int x, y;
    Vector2D() : x(0), y(0) {}
    Vector2D(int m_x, int m_y) : x(m_x), y(m_y) {}
    ~Vector2D() {}
    ...
    // Арифметические операторы сложения векторов, вычетания, умножения и деления
    ...
    case WM_PAINT:
    {
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                ...
                Vector2D shift( 2 * i * (2 * r), 2 * j * (2 * r));
                Vector2D new_position = position + shift;

                Ellipse(hdc, new_position.x, new_position.y, r);
            }
        }
