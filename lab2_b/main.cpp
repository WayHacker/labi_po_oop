...
class Figure {
protected:
    int const m_left, m_top, m_right, m_bottom;
...
public:
    ...
    FigureEllipse(int left, int right, int top, int bottom)
...
public:
    ...
    FigureRectangle(int left, int right, int top, int bottom)
        : Figure(left, right, top, bottom)
    {
    }
};
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE: {
        Figure* ellipse = new FigureEllipse(71, 71, 301, 301);
        Figure* rectangle = new FigureRectangle(401, 51, 601, 301);
        figures.push_back(std::unique_ptr<Figure>(ellipse));
        figures.push_back(std::unique_ptr<Figure>(rectangle));
        break;
    }
    case WM_PAINT: {
        PAINTSTRUCT ps;
        HDC hdc = BeginPaint(hWnd, &ps);
        for (std::unique_ptr<Figure>& i : figures) {
            i->draw(hdc);
        }
        EndPaint(hWnd, &ps);
    } break;
